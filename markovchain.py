...
class MarkovChain:
    ...
    def __init__(self, M, labels, weights, **kwargs):
        """
        ...
        #Define color gradient from red to green as dictionary.
        self.colordict = {-1.0: '#e41515', -0.9: '#e72c2c', -0.8: '#e94444', -0.7: '#ec5b5b', -0.6: '#ef7373', -0.5: '#f28a8a', -0.4: '#f4a1a1', -0.3: '#f7b9b9', -0.2: '#fad0d0', -0.1: '#fce8e8', 0.0: '#ffffff', 0.1: '#ffffff', 0.2: '#e9f9ea', 0.3: '#d3f2d4', 0.4: '#beecbf', 0.5: '#a8e5a9', 0.6: '#92df94', 0.7: '#7cd87e' , 0.8: '#66d269', 0.9: '#51cb53', 1.0: '#25be28'  }
        ...
        #Get normalized changes in weight values from 1st to last for each filter
        self.filter_dvals = self.get_fil_dvals()

        #Set color codes from colordict.
        self.colorcodes = [self.colordict[round(float(i), 1)] for i in self.filter_dvals]
    ...
    def get_fil_dvals(self):
        #Define range of weights in each filter
        ranges = [self.weights[-1,0,i] - self.weights[0,0,i] for i in range(np.shape(self.weights)[2])]
        max_range = np.max(np.abs(ranges))

        norm_ranges = np.round((ranges / max_range), 1)
        norm_ranges = np.append(norm_ranges, 0)
